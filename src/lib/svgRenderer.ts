...
export function generateSVG(options: RenderOptions): string {
  ...
  switch (options.presetId) {
    ...
    case 'pattern-dots':
      ...
      break;
    case 'pattern-grid':
      backgroundSVG = generatePatternGrid(options.palette, width, height);
      cardRadius = 16;
      shadowFilter = '<filter id="shadow"><feDropShadow dx="0" dy="4" stdDeviation="8" flood-opacity="0.1"/></filter>';
      break;
    case 'pattern-noise':
      backgroundSVG = generatePatternNoise(options.palette, width, height);
      cardRadius = 18;
      shadowFilter = '<filter id="shadow"><feDropShadow dx="0" dy="6" stdDeviation="12" flood-opacity="0.12"/></filter>';
      break;
    case 'browser-macos':
